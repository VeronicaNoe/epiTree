...
for (r in 1:length(RE)){
  ...
  for (i in 1:length(feature)){
    ...
    for (j in 1:length(ctxt)){
      ...
      write.csv(testRes.Treat, paste0(baseDir,"/<PATH-TO-tmp>/",RE[r],"_","Treat_",ctxt[j],"_", feature[i], "_DMC_analysis.csv"),row.names = FALSE)
      write.csv(testRes.Acc, paste0(baseDir,"/<PATH-TO-tmp>/",RE[r],"_","Acc_",ctxt[j],"_", feature[i],"_DMC_analysis.csv"),row.names = FALSE)
    }
  }
}
#### summary table
inFiles <- list.files(path=paste0(baseDir,"/<PATH-TO-tmp>"), pattern = "_DMC_analysis.csv$")
toSave<-c()
outdf<-matrix(NA, nrow=length(inFiles), ncol = 7)
  for (i in 1:length(inFiles)){
    minFDR<-0.05
    input<-read.csv(paste0(baseDir,"/<PATH-TO-tmp>/",inFiles[i]),header=TRUE, stringsAsFactors = FALSE, sep=",")
