...
for (r in 1:length(RE)){
  ...
  pdf(paste0(baseDir,"/<PATH-TO-plots/", RE[r], "_Context_methylationLevelsViolinPlot.pdf"), height = 5*numPlotRows, width = 2+length(forPlotOrder)*numPlotCols)
  ...
  invisible(dev.off())
  write.csv(round(allMeans, 3), file.path(paste0(baseDir,"/<PATH-TO-tmp/",RE[r], "_Context_methylationLevelsViolinPlot_means.csv")))
  ...
  ## Do the plot
  dirOut<-paste0(baseDir,"/<PATH-TO-plots/",RE[r], "_Context_methylationLevelsPerFeature.pdf")
  ...
  for (ctxt in allContexts) {
    ...
    write.csv(round(temp, 3), paste0(baseDir,"/<PATH-TO-tmp/",RE[r],"_methylationLevelsPerFeature_",ctxt, ".csv"))
    }
  invisible(dev.off())
}